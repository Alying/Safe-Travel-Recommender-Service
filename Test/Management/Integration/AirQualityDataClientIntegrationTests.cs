...
namespace Test.Management.Integration
{
    ...
    public class AirQualityDataClientIntegrationTests
    {
        ...
        public async Task GetRequest_ValidData_CanReceiveSuccessResponse() 
        {
            ...
            await Assert.ThrowsAsync<Exception>(() => airClient.GetCityAirQualityDataAsync(City.Wrap("ase"), State.Wrap("California"), CountryCode.US, CancellationToken.None));
            await Assert.ThrowsAsync<Exception>(() => airClient.GetDefaultCitiesAsync(State.Wrap("ase"), CountryCode.US, CancellationToken.None));
